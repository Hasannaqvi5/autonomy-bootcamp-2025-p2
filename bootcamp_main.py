...
def main() -> int:
    ...
    # Start worker processes for heartbeat, telemetry and command
    ...
    main_logger.info("All worker processes started")

    # Main process loop: monitor queues and log activity
    # Runs for the duration of RUN_TIME or until disconnection
    starting_time = time.time()
    while time.time() - starting_time < RUN_TIME:
        # Check for generated commands to log
        ...
        if hb_data is not None:
            if "Disconnected" in hb_data:
                main_logger.warning("Drone disconnected!")
                break
            main_logger.info(f"Received status: {hb_data}")

    # Orchestrate clean shutdown
    wc.request_exit()
    main_logger.info("Requested exit for all workers")
